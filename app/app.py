...
def get_user_notes(user_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    read_user(user_id, db)  # Used to check if the user exists
    return crud.get_user_notes(db=db, owner_id=user_id, skip=skip, limit=limit)
